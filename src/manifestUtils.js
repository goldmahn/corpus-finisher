...
export const STAGE_MANIFEST_FILENAME = 'manifest.final.json';
export const STAGE_KEY = 'corpus_finisher';
 ...
 * @returns {string}
 */
export function resolveStageManifestPath(outputFolder) {
  return path.join(outputFolder, STAGE_MANIFEST_FILENAME);
}
 ...
 * @returns {Record<string, unknown>}
 */
export function pickCorpusFinisherQcFields(qcEntry) {
  return {
    status: qcEntry.status ?? null,
    notes: qcEntry.notes ?? [],
    sourceDurationSec: qcEntry.sourceDurationSec ?? null,
    outputDurationSec: qcEntry.outputDurationSec ?? null,
...
export function applyClipQc(manifest, qcEntries) {
  ...
  updated.clips = (Array.isArray(updated.clips) ? updated.clips : []).map((clip) => {
    ...
    return {
      ...
      qc: {
        ...existingQc,
        [STAGE_KEY]: pickCorpusFinisherQcFields(qcEntry),
      },
    };
  });
